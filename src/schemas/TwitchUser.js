...
module.exports = function(utils) {
    ...
    schema.methods.embed = async function(bans = null, communities = null) {
    ...
    schema.methods.message = async function(ephemeral = true) {
    ...
    schema.methods.fetchFollowers = async function () {
    ...
    schema.methods.tmsAffiliation = function() {
    ...
    schema.methods.getMods = async function(includeInactive = false) {
        let data = {streamer: this};
        if (!includeInactive) data.time_end = null;
        return await utils.Schemas.TwitchRole.find(data)
                .populate("streamer")
                .populate("moderator");
    }
    
    schema.methods.getStreamers = async function(includeInactive = false) {
    ...
    schema.methods.getTokens = async function(requiredScopes = []) {
    ...
    schema.methods.getBans = async function() {
        const bans = await global.utils.Schemas.utils.Schemas.TwitchBan.find({chatter: this._id})
                .populate("streamer")
                .populate("chatter")
                .sort({time_start: -1});
        for (let i = 0; i < bans.length; i++) {
    ...
    schema.methods.getFlags = async function() {
        return await utils.Schemas.UserFlag.find({twitchUser: this._id})
            .populate("flag");
    }
    
    schema.methods.getActiveCommunities = async function() {
    ...
    schema.methods.generateCommunityTable = async function(allChannelHistory = null) {
    ...
    schema.methods.fetchModdedChannels = async function(accessToken) {
        ...
        return await utils.Schemas.TwitchRole.find({moderator: this, time_end: null})
            .populate(["streamer","moderator"]);
    }
    
    let migrating = false;
    schema.methods.migrateData = function() {
